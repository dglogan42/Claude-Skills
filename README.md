...
| [`no-ai-slop`](no-ai-slop/SKILL.md) | Edit drafts into sharper, more human writing while preserving the writer's voice, or detect AI-slop patterns without rewriting. Not persona-voiced; a copy-editing utility used as `bubble-tea-news-editor`'s Step 4b QC pass over every persona's generated column. Vendored unmodified from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop) (MIT licensed, see that folder's own `LICENSE`/`README.md`) — not an original skill from this library. |
...
| [`skeletor-obituary-column`](skeletor-obituary-column/SKILL.md) | Write "In Memoriam," a mock-obituary column narrated by Skeletor (Masters of the Universe's villain, Mattel-owned IP), applying his grandiose doom-and-conquest melodrama to the "passing" of explicitly non-living, trivial subjects — a discontinued flavor, a closed café, a losing streak — never real death, never a person. Seed skill — one persona, one format, hard content boundary on the subject matter. |
| [`horatio-mccallister-personals-column`](horatio-mccallister-personals-column/SKILL.md) | Write "Ask the Captain," a lonely-hearts personals/advice column narrated by Horatio McCallister, The Simpsons' sea captain (Fox-owned IP), answering invented local dating questions almost entirely in nautical metaphor with total unearned confidence. Seed skill — one persona, one format, illustrative-only (not a real dating service) boundary. |
