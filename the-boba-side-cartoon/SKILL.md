...
Verified working as a third option (Issue 8's panel was rebuilt this
way) — sidesteps this skill's own documented ImageMagick SVG-writer bug
entirely, since the SVG here is hand-authored, not generated by
`magick ... -draw "..." out.svg`. Also produces cleaner linework and a
genuinely evenly-tiled screentone than the ImageMagick `-draw` path.

1. Author the panel as plain SVG: shapes as `<path>`/`<circle>`/`<rect>`,
   a `<clipPath>` matching the figure's silhouette outline so screentone
   dots (or a genuine scatter-plot-style irregular dot spread, if the
   gag calls for it) stay confined inside the figure without spilling
   past the linework. Keep Step 0's boundaries and Step 2's six-part
   formula in mind while composing — this is a renderer swap, not a
   change to the style rules themselves. The one spot-color accent
   (Step 2, part 3) is just a `fill="#rrggbb"` on whichever shapes carry
   it — no special CSS tagging needed, same as the PNG-only ImageMagick
   path (see Step 3 above on why: raster `<img>` output isn't touched by
   the paper's `svg *:not(.tcolor)` print-CSS rule either way).
2. Rasterize via Inkscape's CLI:
   ```
   inkscape panel.svg --export-type=png \
     --export-filename=panel.png \
     --export-width=W --export-height=H \
     --export-png-antialias=3 \
     --export-background=white --export-background-opacity=1
   ```
   Leave color mode at the default (RGB) here, unlike Hayaku's pure
   black-and-white strips — Boba Side's spot-color accent needs the
   color channels. `--export-png-antialias=3` (max) is what gives the
   visibly cleaner edges over ImageMagick's `-draw` antialiasing.
3. Stamp real 300 DPI metadata afterward, same as every other renderer
   in this library: `magick panel.png -units PixelsPerInch -density 300
   panel-300dpi.png`.
4. Keep the `.svg` source alongside the delivered `.png` in `issues/`
   — a real editable vector source worth keeping, unlike the
   ImageMagick/GDI paths which have no reusable intermediate artifact.
