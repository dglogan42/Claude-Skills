...
> Vendored unmodified from [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop)
> by Peter Yang (MIT licensed — see `LICENSE` and `README.md` in this folder).
> Only this file's frontmatter gained a `user-invocable: true` line for
> consistency with this repo's convention; the working method below is
